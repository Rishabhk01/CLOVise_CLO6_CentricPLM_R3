...
namespace CLOVise
{
	class UpdateProductBOMHandler : public QObject
	{
		Q_OBJECT
		static UpdateProductBOMHandler* _instance;
		UpdateProductBOMHandler(QObject* parent = nullptr);
	public:
		static UpdateProductBOMHandler* GetInstance();
		...
		void updateBom(string _styleId);
		void readExistingBOMDetailsAndShow(json  _bomLineJson);
		bool m_bomCreatedInPlm;
		string m_apparelBomId;
		bool m_updateBom;
		string m_updatelatestRevision;
	private slots:
