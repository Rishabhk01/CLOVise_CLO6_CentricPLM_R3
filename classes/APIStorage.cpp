...
namespace CLOAPISample
{
	...
	void APIStorage::setFileSizeForAllItems()
	{
		...
		for (size_t i = 0; i < m_MaterialAPIItemList.size(); ++i)
		{
			QString filePath;
			GetFilePathWithID(m_MaterialAPIItemList[i]->itemID, filePath);
			m_MaterialAPIItemList[i]->filesize = getFileSize(filePath);
		}
		for (size_t i = 0; i < m_TrimAPIItemList.size(); ++i)
		{
			QString filePath;
			GetFilePathWithID(m_TrimAPIItemList[i]->itemID, filePath);
			m_TrimAPIItemList[i]->filesize = getFileSize(filePath);
		}
		for (size_t i = 0; i < m_ProductAPIItemList.size(); ++i)
		{
			QString filePath;
	...
	{
		Logger::Logger("APIStorage GetSearchList start");
		vector<LibraryAPIItem> searchResult;
		if (searchKeyValues[META_DATA_KEY_0_DATA_TYPE].toString().compare("MATERIAL", Qt::CaseInsensitive) == 0)
		{
			for (auto it : m_MaterialAPIItemList)
			{
				searchResult.push_back(*it);
			}
		}
		else if (searchKeyValues[META_DATA_KEY_0_DATA_TYPE].toString().compare("TRIM", Qt::CaseInsensitive) == 0)
		{
			for (auto it : m_TrimAPIItemList)
			{
				searchResult.push_back(*it);
			}
		}
		...
		{
			for (auto it : m_LibraryAPIItemList)
			{
				searchResult.push_back(*it);
			}
		}
		...
		/*for (size_t i = 0; i < m_LibraryAPIItemList.size(); ++i)
		{
			if (m_LibraryAPIItemList[i]->itemName.contains(searchText, Qt::CaseInsensitive))
			{
				...
				{
					Logger::Logger(" searchKeyValue key " + iter.key().toStdString());
					Logger::Logger(" searchKeyValue value " + iter.value().toString().toStdString());
		...
		/*for (auto it : searchResult)
		{
			Logger::Logger("APIStorage searchResult :::  " + it.itemName.toStdString());
		}*/
		Logger::Logger("APIStorage GetSearchList end");
		return searchResult;
	}

	bool APIStorage::GetItemWithID(const QString& itemId, LibraryAPIItem& result)
	{
		Logger::Logger("APIStorage::GetItemWithID start");
		Logger::Logger("APIStorage::GetItemWithID itemId ::::::::::::::::::" + itemId.toStdString());
		...
		if (returnValue == false)
		{
			for (size_t i = 0; i < m_MaterialAPIItemList.size(); ++i)
			{
				if (m_MaterialAPIItemList[i]->itemID.compare(itemId, Qt::CaseInsensitive) == 0)
				{
					result = *m_MaterialAPIItemList[i];
					returnValue = true;
					break;
				}
			}
		...
		Logger::Logger("APIStorage::GetItemWithID end");
		return returnValue;
	}
	...
	bool APIStorage::GetFilePathWithID(const QString& itemId, QString& resultFilePath)
	{
		...
		{
			resultFilePath = QString::fromStdString(DirectoryUtil::GetSampleStorageDirectory()) + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.itemPath);
			//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + curItem.sampleItemData.itemPath;
			return true;
		}
	...
	bool APIStorage::GetIconThumbnailPathWithID(const QString& itemId, QString& resultFilePath)
	{
		...
		if (GetItemWithID(itemId, curItem))
		{
			if(curItem.sampleItemData.iconThumbnailPath != "")
				resultFilePath = QString::fromStdString(DirectoryUtil::GetSampleStorageDirectory()) + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.iconThumbnailPath);
				//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + curItem.sampleItemData.iconThumbnailPath;
			else
				resultFilePath = QString::fromStdString(DirectoryUtil::GetSampleStorageDirectory()) + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.itemPath);
				//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + curItem.sampleItemData.itemPath;
	...
	bool APIStorage::GetPreviewThumbnailPathWithID(const QString& itemId, QString& resultFilePath)
	{
		...
		if (GetItemWithID(itemId, curItem))
		{
			if (curItem.sampleItemData.previewThumbnailPath != "")
				resultFilePath = QString::fromStdString(DirectoryUtil::GetSampleStorageDirectory()) + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.previewThumbnailPath);
				//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + curItem.sampleItemData.previewThumbnailPath;
			else
				resultFilePath = QString::fromStdString(DirectoryUtil::GetSampleStorageDirectory()) + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + GetOnlyFileName(curItem.sampleItemData.itemPath);
				//resultFilePath = SAMPLE_STORAGE_DIRECTORY + curItem.metaData[META_DATA_KEY_0_DATA_TYPE].toString().toLower() + "/" + curItem.sampleItemData.itemPath;
...
//		fabricItem1->dateTime = "2019-01-20T11:11:21";
	...
	void APIStorage::SetPLMAssetItemList(QString _itemDataType)
	{
		
		string assetPath = UTILITY_API->GetCLOAssetFolderPath(true) + "CLOPLMPlugin_Assets/plm/";
		if (_itemDataType == SIDE_MENU_PRODUCT_ID)
			generateAPIItemListForPLMProduct(assetPath + "product/", _itemDataType);
		if (_itemDataType == SIDE_MENU_MATERIAL_ID)
			generateAPIItemListForPLMMaterial(assetPath + "material/", _itemDataType);
		if (_itemDataType == SIDE_MENU_COLOR_ID)
	...
	void APIStorage::generateAPIItemListForPLMColor(string _directoryPath, QString _itemDataType)
	{
		...
		for (int fileListCount = 0; fileListCount < fileList.size(); fileListCount++)
		{
			...
			if (COLOR_SUPPORTING_LIST.contains(QString::fromStdString(Helper::GetFileExtension(fileName.toStdString())), Qt::CaseInsensitive))
			{
				...
				string objectId = "";
				LibraryAPIItem* setItem = newLibraryAPIItemWithRandomizedDate();
				if (FormatHelper::HasContent(metadataStr))
				{
					json metadataJson = json::parse(metadataStr);
					objectId = Helper::GetJSONValue<string>(metadataJson, OBJECT_ID, true);
				...
				setItem->metaData[COLOR_DATA_ID_KEY + META_DATA_KEY_1_DATA_TYPE] = PRINTS_AND_PATTERN_DATA_ID_KEY;
				setItem->filesize = fileSize;
				setItem->dateTime = "2019-11-01T13:10:48";

				if (!Helper::CheckFileExist(fileName, setItem))
					m_LibraryAPIItemList.push_back(setItem);
			}
		}
	}
	...
	void APIStorage::generateAPIItemListForPLMProduct(string _directoryPath, QString _itemDataType)
	{
		...
		for (int fileListCount = 0; fileListCount < fileList.size(); fileListCount++)
		{
			...
			if (PRODUCT_SUPPORTING_LIST.contains(QString::fromStdString(Helper::GetFileExtension(fileName.toStdString())), Qt::CaseInsensitive))
			{
				...
				float fileSize = (float)Helper::GetFileSize(filePath.toStdString());
				setItem->metaData[META_DATA_KEY_0_DATA_TYPE] = _itemDataType;
				setItem->metaData[PRODUCT_DATA_ID_KEY + META_DATA_KEY_1_DATA_TYPE] = DOCUMENT_DATA_ID_KEY;
				setItem->itemName = fileName;
				...
				if (!Helper::CheckFileExist(fileName, setItem))
					m_ProductAPIItemList.push_back(setItem);
			}
		}
	}
	...
	void APIStorage::generateAPIItemListForPLMMaterial(string _directoryPath, QString _itemDataType)
	{
		...
		for (int fileListCount = 0; fileListCount < fileList.size(); fileListCount++)
		{
			...
			//Logger::Logger("APIStorage generateAPIItemListForPLMMaterial fileName :   " + fileName.toStdString());
			
			if (MATERIAL_SUPPORTING_LIST.contains(QString::fromStdString(Helper::GetFileExtension(fileName.toStdString())), Qt::CaseInsensitive))
			{
				string metadataStr = UTILITY_API->GetAPIMetaData(filePath.toStdString());
				string objectId = "";
				LibraryAPIItem* setItem = new LibraryAPIItem();
				...
				float fileSize = (float)Helper::GetFileSize(filePath.toStdString());
				setItem->metaData[META_DATA_KEY_0_DATA_TYPE] = _itemDataType;
				setItem->metaData[MATERIAL_DATA_ID_KEY + META_DATA_KEY_1_DATA_TYPE] = FABRIC_DATA_ID_KEY;
				...
				if (!Helper::CheckFileExist(fileName, setItem))
				{
					m_MaterialAPIItemList.push_back(setItem);
				}
					//m_LibraryAPIItemList.push_back(setItem);
				/*for (auto& it : m_MaterialAPIItemList)
				{
					Logger::Logger("APIStorage m_MaterialAPIItemList fileName :   " + it->itemName.toStdString());
				}*/
			}
		}
		Logger::Logger("APIStorage generateAPIItemListForPLMMaterial end");
	}

	void APIStorage::generateAPIItemListForPLMTrims(string _directoryPath, QString _itemDataType)
	{
		_directoryPath = Helper::FindAndReplace(_directoryPath, LEFT_DOUBLE_SLASH, WRIGHT_SINGLE_SLASH);
		QDir dir(QString::fromStdString(_directoryPath));
		QFileInfoList fileList = dir.entryInfoList(QDir::Files, QDir::Time);
		m_TrimAPIItemList.clear();
		for (int fileListCount = 0; fileListCount < fileList.size(); fileListCount++)
		{
			QString filePath = fileList.at(fileListCount).filePath();
			QString fileName = fileList.at(fileListCount).fileName();
			...
			if (TRIM_SUPPORTING_LIST.contains(QString::fromStdString(Helper::GetFileExtension(fileName.toStdString())), Qt::CaseInsensitive))
			{
				LibraryAPIItem* setItem = new LibraryAPIItem();
				string objectId = "";
				string metadataStr = UTILITY_API->GetAPIMetaData(filePath.toStdString());
				...
				float fileSize = (float)Helper::GetFileSize(filePath.toStdString());
				setItem->metaData[META_DATA_KEY_0_DATA_TYPE] = _itemDataType;
				setItem->metaData[MATERIAL_DATA_ID_KEY + META_DATA_KEY_1_DATA_TYPE] = TRIM_DATA_ID_KEY;
				setItem->itemName = fileName;
				setItem->sampleItemData.itemPath = filePath;
				setItem->sampleItemData.iconThumbnailPath = filePath;
				setItem->sampleItemData.previewThumbnailPath = filePath;
				...
				if (!Helper::CheckFileExist(fileName, setItem))
					m_TrimAPIItemList.push_back(setItem);
			}
		}
	}
}